...
void ChemODERK::init()
{
  ...
  if (volume == 0.0) error->all("Must set volume for run style ode/rk");
  ...
  // setup the rkf integrator

  abserr = 2.0*sqrt(d_epsilon());
  relerr = 2.0*sqrt(d_epsilon());

  flag = 1;
  t = simulator->ctime;
  flag = rkf45_d(nspecies,ccount,ccopy,&t,t,&relerr,abserr,flag);
}
...
void ChemODERK::reactions()
{
  ...
  if (flag == 7) flag = 2;
  flag = 
    rkf45_d(nspecies,ccount,ccopy,&t,t+dt,&relerr,abserr,flag);
}
...
/* ----------------------------------------------------------------------
   function call for updating concentrations by t
...
void ChemODERK::f(double t, double conc[], double *deriv)
{
  int i, j;
  double delta;

  for (j = 0; j < nspecies; j++) deriv[j] = 0;
  ...
  for (i = 0; i < nreactions; i++) {
    if (nreactant[i] == 0) delta = rate[i];
    else if (nreactant[i] == 1) delta = conc[reactants[i][0]] * rate[i];
    else {
      if (reactants[i][0] == reactants[i][1]) 
	delta = 0.5 * conc[reactants[i][0]] * 
	  conc[reactants[i][1]] * rate[i];
      else
	delta = conc[reactants[i][0]] * 
	  conc[reactants[i][1]] * rate[i];
    }
    
    for (j = 0; j < nreactant[i]; j++)
      deriv[reactants[i][j]] -= delta * wreactant[i][j];
    for (j = 0; j < nproduct[i]; j++)
      deriv[products[i][j]] += delta * wproduct[i][j];
  }
...
/* ----------------------------------------------------------------------
   Runge-Kutta variable step size ODE solver
   Purpose:
...
double ChemODERK::d_max ( double x, double y )
...
double ChemODERK::d_min ( double x, double y )
...
double ChemODERK::d_sign ( double x )
...
void ChemODERK::fehl_d ( int neqn, 
...
int ChemODERK::rkf45_d (int neqn, 
			double y[], double yp[], double *t, 
...
//  accuracy requests.
//
  relerr_min = 2.0 * d_epsilon ( ) + remin;
//
...
//
  ...
  if ( init == 0 )
  {
    ...
    h = d_max ( h, 26.0 * eps * d_max ( fabs ( *t ), fabs ( dt ) ) );
...
//
      fehl_d ( neqn, y, *t, h, yp, f1, f2, f3, f4, f5, f1 );
      nfe = nfe + 5;
//
